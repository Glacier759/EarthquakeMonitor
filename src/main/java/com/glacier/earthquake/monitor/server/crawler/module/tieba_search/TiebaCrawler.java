...
public class TiebaCrawler extends Crawler {
        ...
        try {
            ...
            do {
                try {
                    ...
                    //遍历所有的搜索结果
                    Elements elements = document.select("div[class=s_post]");
                    ...
                    for (Element element : elements) {
                        try {
                            ...
                            //再次依据白名单做以筛选
                            if (JudgeFilter.isMeetWhiteList(postLink)) {
                                continue;
                            }

                            if ( Scheduler.getRecordBySignVale(postLink, rule_id, SpiderInfo.FILTER_DISASTER) != null ) {
                            ...
                            Scheduler.insertRecord(Scheduler.SIGN_URL, postLink, Scheduler.SERVICE_TIEBA_SEARCH, rule_id, SpiderInfo.FILTER_DISASTER);
                            ...
                            if ( summary != null ) {
                                ...
                                spiderInfo.setOrigin(SystemConfig.CONFIG_TYPE_TIEBA_SEARCH);
                                //设置好属性后插入数据库
    ...
    private Document next(Document document) {
        try {
            if ( document.select("a[class=next]").size() > 0 ) {
