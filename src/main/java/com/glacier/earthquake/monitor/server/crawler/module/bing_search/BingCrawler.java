...
public class BingCrawler extends Crawler {
        ...
        try {
            ...
            do {
                try {
                    ...
                    //遍历所有的搜索结果
                    Elements elements = document.select("li[class=b_algo]");
                    logger.info("[解析] - 本页获得搜索结果 " + elements.size() + " 条(粗略结果)");
                    for (Element element : elements) {
                        try {
                            String resultLink = element.select("h2").select("a[href]").attr("abs:href");
                            ...
                            Scheduler.insertRecord(Scheduler.SIGN_URL, resultLink, Scheduler.SERVICE_BING_SEARCH, rule_id, SpiderInfo.FILTER_DISASTER);
                            ...
                            if ( summary != null ) {
                                ...
                                spiderInfo.setOrigin(SystemConfig.CONFIG_TYPE_BING_SEARCH);
                                //设置好属性后插入数据库
    ...
    private Document next(Document document) {
        try {
            if ( document.select("a[title]").size() > 0 && document.select("a[title]").text().contains("下一页") ) {
